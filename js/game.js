/* game.js
   Core Game Logic: handles dish logic, collisions, scoring, etc.
*/

import { SpritePlayer } from './spritePlayer.js';

export const Game = {
  player: null,  // Assigned in index.html after import
  canvas: null,
  ctx: null,
  ...
  init() {
    ...
    this.loadAssets();
    this.player = this.player || new SpritePlayer(); // fallback if not assigned
    ...
    window.addEventListener('keydown', e => {
      switch(e.key.toLowerCase()) {
        ...
        case 'e':
          ...
          // Only handle pick-up/put-down if not currently washing
          if (!this.player.isWashing) this.handleInteraction();
          break;
        case 'escape':
          // Toggle pause if game started
          if (this.gameStarted) {
  ...
  drawScene() {
    ...
    if (this.images.sink.complete) {
      ...
      this.ctx.drawImage(this.images.sink, 0, 0,
        this.zones.sink.width, this.zones.sink.height);
      this.ctx.restore();
    }
  ...
  updatePrompt() {
    ...
    if (this.player.heldItem) {
      if (this.player.heldItem.state === 'dirty' && this.checkCollision(rectPlayer, this.zones.sink)) {
  ...
  handleInteraction() {
    ...
    // If holding clean => store on shelf
      ...
      if (this.checkCollision(rectPlayer, shelfZone) &&
  ...
  removeDishes() {
    ...
    Object.keys(this.storedDishes).forEach(type => {
      if (this.storedDishes[type] >= this.MIN_DISHES_BEFORE_REMOVAL &&
